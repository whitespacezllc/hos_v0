...
// phrase over an editorial row of photographs that quietly exchange
...
// heights, swapping on their own beats. It first lived in the home's
// "A Journey of Wisdom and Wellness"; the words and the pool of photographs
 ...
 */
function useRotatingSlots(pool: string[], enabled: boolean) {
...
export function IntroArrangement({
  copy,
  pool,
}: {
  copy: IntroCopy;
  ...
  const indices = useRotatingSlots(pool, inViewport && pageVisible && reducedMotion === false);
